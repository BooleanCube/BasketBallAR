...
namespace MagicLeap.SetupTool.Editor.Setup
{
    ...
    public class ImportMagicLeapSdkStep : ISetupStep
    {
        ...
        public async void CheckVersion()
        {
            var getPackageInfoResult = await PackageUtility.GetPackageInfoAsync(MAGIC_LEAP_PACKAGE_ID);
              ...
              if (_installedFromRegistry)
              {
                  ...
                  var isCurrentVersion = versionComparer.Compare(getPackageInfoResult.versions.latest,getPackageInfoResult.version) <=0;
                  _isCurrent = isCurrentVersion;
                  if ((!isCurrentVersion))
                  {
                      _sdkPackageVersion = string.Format(UPDATE_PACKAGE_TOOLTIP, getPackageInfoResult.version, latestVersion);
                      return;
                  }
              }
              else
              {
                  ...
                  if (directoryInfo != null)
                  {
                      ...
                      if ((!isCurrentVersion))
                      {
                          _sdkPackageVersion = string.Format(UPDATE_PACKAGE_TOOLTIP, getPackageInfoResult.version, directoryInfo.Name);
                          return;
                      }
                  }
              }
        ...
        /// </summary>
        private async void CheckForMagicLeapSdkPackage(Action onFinished = null)
        {
